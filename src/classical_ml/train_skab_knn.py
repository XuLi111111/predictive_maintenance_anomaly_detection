...
# k values to explore during hyperparameter tuning on validation set
...
def tune_on_val(
    ...
    Selects the k with best F1-score on val.

    KNN does not support class_weight — we rely on the natural class
    distribution (~36% faults) which is manageable without SMOTE here,
    unlike the Zenodo dataset which had 0.3% faults.
    ...
    """
    print(f"\n  Tuning k on validation set (selecting by best F1-score)...")
    print(f"  {'k':<8} {'Val Recall (%)':>15} {'Val F1 (%)':>12} {'Val Precision (%)':>18}")
    print("  " + "-" * 58)
    ...
    for k in k_values:
        ...
        tuning_log.append({
            "k": k, "val_recall": recall,
            "val_f1": f1, "val_precision": precision,
        })
        print(f"  {k:<8} {recall*100:>14.2f}% {f1*100:>11.2f}% {precision*100:>17.2f}%")

        if f1 > best_f1:
            best_f1 = f1
            best_k  = k

    print(f"\n  Selected k = {best_k}  (best val F1 = {best_f1*100:.2f}%)")
...
def save_results(
    ...
    with open(filename, "w") as f:
        ...
        f.write(f"Script         : train_skab_knn.py\n")
        f.write("=" * 60 + "\n\n")
        ...
        for entry in tuning_log:
            f.write(f"  k={entry['k']:<6}  Val Recall={entry['val_recall']*100:.2f}%  "
        ...
        f.write(f"  imbalance     : none needed (~36% fault rate)\n\n")
        ...
        f.write(f"\n  Observations:\n")
        f.write(f"  KNN classifies each window by majority vote from k nearest neighbours.\n")
        f.write(f"  SKAB has ~36% fault rate — much more balanced than Zenodo (0.3%).\n")
        f.write(f"  No SMOTE needed here. StandardScaler ensures fair distance computation.\n")
        f.write(f"  KNN prediction is slow on large datasets due to distance computation.\n")
        f.write("=" * 60 + "\n")
...
def save_model(model, scaler, models_dir: str) -> None:
    ...
    model_path  = os.path.join(models_dir, "model_knn_skab.pkl")
    scaler_path = os.path.join(models_dir, "scaler_knn_skab.pkl")
...
def main():
    print("=" * 60)
    print("  K-Nearest Neighbours (KNN) — SKAB Fault Detection Training")
    print("=" * 60)
    ...
    best_k, tuning_log = tune_on_val(X_train, y_train, X_val, y_val, K_VALUES)

    # --- Train final model ---
    print(f"\n[3/5] Training final model with k={best_k}...")
    ...
    save_results(
        ...
        best_k, K_VALUES, tuning_log, metrics, y_test, train_time,
    )
